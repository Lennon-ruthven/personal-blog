...
const ToolsCard = ({ name, description, link, id, labels }) => {
  ...
  return (
    ...
    >
      <div className="animate-tilt absolute -inset-0.5 rounded-lg bg-gradient-to-r from-lime-600 to-lime-500 opacity-10 blur transition duration-1000 group-hover:opacity-75 group-hover:duration-200"></div>
        ...
        <div className="h-full px-2 py-4">
          ...
          <span className="my-2 inline-flex w-full items-center justify-between">
            <span className="inline-block rounded border border-gray-700 px-2 py-1 text-xs font-medium">
              {labels.map((tag) => (
                <a key={tag} className="mr-3 text-xs font-medium uppercase hover:text-lime-600 ">
                  {tag.split(' ').join('-')}
                </a>
              ))}
            </span>
